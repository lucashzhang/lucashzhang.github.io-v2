...
function fromNotionObject(notionPage) {
  return {
    ...
    Tags:
      notionPage.properties.Tags?.multi_select?.map((item) => item.name) ?? [],
  };
}

export async function get() {
  const { NOTION_TOKEN, NOTION_DATABASE, NOTION_DATABASE_DATE } = process.env;
